...
func NewVoterAPI(location string, pubAPIurl string) (*VoterAPI, error) {
...
func (p *VoterAPI) GetVoter(c *gin.Context) {
	...
	var votrs schema.Voter
	err = json.Unmarshal(pubBytes.([]byte), &votrs)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "cached data seems to be wrong type"})
		return
	}

	c.JSON(http.StatusOK, votrs)
}

func (p *VoterAPI) GetVoters(c *gin.Context) {

	var pubList []schema.Voter
	var pubItem schema.Voter
...
func (p *VoterAPI) DeleteVoter(c *gin.Context) {
	voterid := c.Param("id")
	if voterid == "" {
		c.JSON(http.StatusBadRequest, gin.H{"error": "No vote ID provided"})
		return
	...
	if exists == 0 {
		c.JSON(http.StatusNotFound, gin.H{"error": "vote not found in cache"})
		return
	}
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to delete vote from cache"})
		return
	}
...
func (p *VoterAPI) getItemFromRedis(key string, pub *schema.Voter) error {
